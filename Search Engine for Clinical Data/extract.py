...
# This function is used to extract the desired data from the input xml files. After extracting the desired data from each input xml file, it is stored in an ordered dictionary. This dictionary is then passed to the elastic_index function which will index the data in Elasticsearch.
...
def extract_data_xml():
       #Path to xml file
       list_of_files = glob.glob('clinicaltrials_xml/000/00000' + '/*.xml')
       ...
       #This for loop iterates over each input file. Within each try-except block we try to extract the data 
       #from one particular xml field. This extracted data is stored in an ordered dictionary with key 
       #as the field name and value as the extracted data.
       #Currently the following fields are extracted: nct_id, brief_title, brief_summary,
       #detailed_description, overall_status, condition, eligibility, gender, gender_based, minimum_age, maximum_age, 
       #keyword, mesh_term Not all the files contain all the fields we desire, hence the multiple try-except blocks.
...
# It receives a counter value and the dictionary containing the extracted data from 
# the extract_data_xml function. The counter value is used as the id during indexing.
# 
...
#https://towardsdatascience.com/getting-started-with-elasticsearch-in-python-c3598e718380 : Used for elastic search
...
if __name__ == '__main__':
       #Create connection to Elasticsearch listening on localhost port 8889. It uses the Python Elasticsearch API which is the official low-level client for Elasticsearch.
       try:
